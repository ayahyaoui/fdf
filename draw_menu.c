...
int         put_title(t_fdf *fdf, const char title[], int y, int color)
{
    ...
    mlx = fdf->mlx;
    mlx_string_put(mlx->mlx_ptr, mlx->win, mlx->menu_img->width / 5, y, color, (char*)title);
    return 1;
}

void        draw_option(t_fdf *fdf, const t_menu type[], char name[GRID_NB][20], const t_option options[])
{
    int     i;
    t_mlx   *mlx;
    t_img   *img;
    ...
    {
        if (type[i] == TITLE)
            put_title(fdf, name[i], i * mlx->size_y / GRID_SIZE,  set_b(0, 255));
        if (type[i] == SUB_TITLE)
            put_subtitle(fdf, mlx, name[i], i * mlx->size_y / GRID_SIZE, options[i]);
        if (type[i] == GRADUATION)
        {
            put_graduation(fdf, name[i], name[i+1],  i * mlx->size_y / GRID_SIZE);
            i++;
        }
        i ++;
    }
...
{   
    ...
    char name[GRID_NB][20] = {"", "", "", "PROJECTION", "ORTHO",  "ISOMETRIC",  "PARALLELE", "", 
